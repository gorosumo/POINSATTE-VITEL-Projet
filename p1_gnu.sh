...
set output "p1_png" #On indique que le fichier de sortie doit s'appeler t1_png.
set title "Pression p1" #On définie "Température t1" comme titre du graphique.
...
set ylabel "Pression" #On définie "Température" comme nom de l'axe des ordonnées.
...
plot 'press4.csv' using 0:3:4 with filledcurve fc rgb Shadecolor_a title "Pression minimale à maximale", '' using 0:2:xtic(1) with linespoint lw 3 lc rgb Shadecolor_b title "Pression moyenne" #on utilise le dossier " " pour créer deux courbes : une avec la température minimale à maximale en orange, qui utilisera des courbes remplis à l'aide des colonnes 1, 2 et 4 du fichier. Et l'autre avec la température moyenne en rouge, qui utilisera des courbes à l'aide des colonnes 1 et 3 du fichier.
